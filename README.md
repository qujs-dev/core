...
```
Этот код должен выполниться до любых вызовов `Que(...)`. Поэтому он подключается самым первым, синхронно, в `<head>` до любых других скриптов. Так все вызовы `Que` и события, произошедшие до загрузки основного ядра, будут сохранены и обработаны позже.
